...
def main(merged_df):
    ...
    def calculate_investment_scores(df):
        df = calculate_moving_averages(df, 50)
        df = calculate_moving_averages(df, 200)
        ...
        if 'ema' not in df.columns or 'sma' not in df.columns:
            st.error("Columns 'EMA' and 'SMA' are required but missing in the data.")
            return df

        df['ema_sma_score'] = df.apply(lambda row: 1 if row['ema'] > row['sma'] else -1, axis=1)
        df['rsi_score'] = df['rsi'].apply(lambda x: 1 if x < 30 else (-1 if x > 70 else 0))
        
        df['investment_score'] = 0.3 * df['price_momentum'] + \
                                0.2 * df['volume_score'] + \
                                0.2 * df['volatility_score'] + \
                                0.2 * df['ema_sma_score'] + \
                                0.1 * df['rsi_score']
    ...
    with col4:
        ...
        st.markdown(
            f"""
            <div style='height:350px; width:100%;background-color:#F5F5F5;padding:10px;border-radius:10px;text-align:center'>
    ...
    <div style="background-color:#f9f9f9; padding:20px; border-radius:10px;">
        ...
        <h3 style="color:#4CAF50; margin-top:20px;">4. EMA vs SMA Score:</h3>
        <p style="font-size:16px; line-height:1.6;">
            ...
            <strong>Reason:</strong> If EMA is above SMA, it indicates bullish momentum. If EMA is below SMA, it suggests bearish trends.
        </p>
        ...
        <p style="font-size:16px; line-height:1.6;">
            ...
            <strong>Reason:</strong> RSI below 30 indicates potential undervaluation (buy signal), while RSI above 70 suggests overvaluation (sell signal).
        </p>
        ...
        <p style="font-size:16px; line-height:1.6;">
            ...
            <strong>Formula:</strong> 
            <code style="background-color:#e6e6e6; padding:2px 5px; border-radius:4px;">
                0.3 * Price Momentum + 0.2 * Volume Score + 0.2 * Volatility Score + 0.2 * EMA vs SMA Score + 0.1 * RSI Score
            </code><br>
